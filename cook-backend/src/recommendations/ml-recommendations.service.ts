...
export class MLRecommendationsService {
  ...
  /**
   * Generar recomendaciones usando técnicas de Machine Learning
   */
  async getMLRecommendations(
    ...
    for (const itemVector of itemVectors) {
      ...
      if (predictedRating > 3.0) { // Solo recomendar items con rating predicho > 3
        predictions.push({
  ...
  private async buildUserVector(userId: number): Promise<UserVector> {
    ...
    this.extractFavoriteFeatures(favoritos, features, preferences);
    ...
    // Características temporales (dimensiones 40-49)
  ...
  private extractFavoriteFeatures(
    ...
    features[25] = favoritos.length; // Total de favoritos
  ...
  private extractTemporalFeatures(actividades: any[], features: number[]): void {
    ...
    // Mapear a features (dimensiones 40-49)
    diasSemana.forEach((count, dia) => {
      features[40 + dia] = count / actividades.length; // Normalizar
    });
    ...
    features[47] = actividadReciente / actividades.length;
  }
